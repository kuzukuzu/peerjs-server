...
require('./models.js').init_models();
var Cast = mongoose.model('Cast');
var ClientTree = mongoose.model('ClientTree');
...
app._initializeHTTP = function() {
  var self = this;
  // ストリームを配信する子クライアントの最大数
  this.child_max_num = 3;
  ...
  // link cast with mongo's Cast
  this.post('/:key/:id/:castid/gencast', function(req, res, next) {
    var key = req.params.key;
    var id = req.params.id;
    var castid = req.params.castid;
    Cast.update({_id: castid}, {$set: {masterid: id}}, function(err, raw) {
      if (raw.n && raw.n > 0) {
        // TODO: ここを別関数化したい
        var master = new ClientTree();
        master.cast_id = castid;
        master.client_id = id;
        master.node_path = '/';
        master.child_num = 0;
        master.depth = 0;
  ...
  // Decide client who give you stream.
  this.get('/:key/:id/:castid/castid', function(req, res, next) {
    var key = req.params.key;
    var id = req.params.id;
    var castid = req.params.castid;

    // search for id, and return parent
    // TODO: synchronizing
    ClientTree.find({client_id: id}, function(err, docs) {
      if (docs.length > 0) {
        var path = docs[0].node_path;
        var found = path.match(/\/([^/]+)\/$/);

        ClientTree.find({client_id: found[1]}, function(err, docs) {
          if (docs.length > 0) {
            res.send(found[1]);
          } else {
            ClientTree.rebaseClient({client_id: id}, function() {});
          }
        });
      } else {
        ClientTree.find({cast_id: castid}, {}, {sort: {depth: 1}}, function(err, docs) {
          if (docs.length === 0) {
            res.sendStatus(404);
            return;
          }
          ...
          docs.every(function(doc) {
            // Find suitable client to be parent
            if (doc.child_num < self.child_max_num) {
              // increments child_num
              // Notice that allows collision
              ClientTree.update({_id: doc._id.toString()}, {$inc: {child_num: 1}}, function(err) { });

              // insert new client info
              // TODO: to fix synchronizing with parent
              var client = new ClientTree();
              client.cast_id = castid;
              client.client_id = id;
              client.node_path = doc.node_path + doc.client_id + '/';
              client.child_num = 0;
              client.depth = doc.depth + 1;
              client.save(function(err) {
                if (err) {
                  res.sendStatus(500);
                } else {
                  res.send(doc.client_id);
                }
              });
              return false;
            }
