...
app._initializeHTTP = function() {
  ...
  this.post('/:key/:id/:castid/gencast', function(req, res, next) {
    ...
    Cast.update({_id: castid}, {$set: {masterid: id}}, function(err, raw) {
      if (raw.n && raw.n > 0) {
        // TODO: ここの処理をmodelに移したい
        var master = new ClientTree();
        master.cast_id = castid;
        master.parent_client_id = null; // rootはparentをnullにする
        master.child_client_id = id;
  ...
  this.get('/:key/:id/:castid/castid', function(req, res, next) {
    ...
    ClientTree.find({child_client_id: id}, function(err, docs) {
      if (docs.length > 0) {
        var parent_id = docs[0].parent_client_id;
        ClientTree.find({child_client_id: parent_id}, function(err, docs) {
          if (docs.length > 0) {
            res.send(parent_id);
          } else {
            ClientTree.rebaseClient({cast_id: castid}, function(err, parent_id) {
              res.send(parent_id);
            });
          }
        });
      } else {
        ClientTree.findParentCandidate({cast_id: castid}, function(err, parent_id) {
          if (err) {
            res.sendStatus(404);
          } else {
            var entry = new ClientTree();
            entry.cast_id = castid;
            entry.parent_client_id = parent_id;
            entry.child_client_id = id;
            entry.save();
            res.send(parent_id);
          }
        });
